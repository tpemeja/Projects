    ...
    for i in range(10**k,10**(k+1),10**k):
        ...
        for mode in range(nb_mode):
            ...
            for j in range(moyenne):
                 process = subprocess.Popen(["./install/bin/"+list_mode[mode],str(i),str(nb_yield)], stdout=subprocess.PIPE)
...
plt.title("Performance avec "+ str(nb_yield)+" yield")
plt.legend()
plt.show()
...
for k in range(Puissance_max):
    for i in range(10**k,10**(k+1),10**k):
        ...
        for mode in range(nb_mode):
            result = 0
            for j in range(moyenne):
                process = subprocess.Popen(["./install/bin/"+list_mode[mode],str(nb_thread),str(i)], stdout=subprocess.PIPE)
                pout, perr = process.communicate()
                result += float(pout.decode("utf-8").split(" ")[int(pos)])
        result = (result/moyenne)
        Ly_mode[mode].append(result)
...
plt.ylabel('Temps en us')
plt.title("Performance avec "+str(nb_thread)+" threads")
plt.legend()
plt.show()
