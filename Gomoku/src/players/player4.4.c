/**
 * @file player4.4c
 * This player implements the heuristic3.h and play with a minmax strategy with a maximal depth and a number of childs maximum. Threads are also used in order to speed computing
 */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
struct move_t play(struct col_move_t const previous_moves[], size_t n_moves){
  ...
  int first_cursor = self.h->cursor; // = 0
  //printf("%d\n",first_cursor);
  generating_moves(self.h, self.color);
  struct move_t m = self.h->stack[first_cursor];
  //printf("better move : %ld %ld\n", m.row, m.col);
  int my_force = heuristic_potential_point(self.h, m);
  if (my_force >= MAX_VAL){
    self.h->cursor = 0; // vider la stack
    //printf("%ld %ld is wining\n", m.row, m.col);
    return m;
  }
